...
ifredhat=$(cat /etc/issue | grep -i redhat)
ifcentos=$(cat /etc/issue | grep -i centos)
ifubuntu=$(cat /etc/issue | grep -i ubuntu)
ifdebian=$(cat /etc/issue | grep -i debian)
...
if [ "$ifredhat" != "" ];then
  ...
  yum makecache
  yum -y remove mysql MySQL-python perl-DBD-MySQL dovecot exim qt-MySQL perl-DBD-MySQL dovecot qt-MySQL mysql-server mysql-connector-odbc php-mysql mysql-bench libdbi-dbd-mysql mysql-devel-5.0.77-3.el5 httpd php mod_auth_mysql mailman squirrelmail php-pdo php-common php-mbstring php-cli &> /dev/null
  yum -y install gcc gcc-c++ gcc-g77 make libtool autoconf patch automake libxml2 ncurses ncurses-devel libtool-ltdl-devel libtool-ltdl libmcrypt libmcrypt-devel libpng libpng-devel openssl openssl-devel curl libxml2 ncurses ncurses-devel libtool-ltdl-devel libtool-ltdl autoconf automake libaio* tar wget openssh-server libxml2-devel unzip curl-devel libjpeg-devel
  iptables -P INPUT ACCEPT
  iptables -F
